...
public class Grid implements Iterable<Cell> {
    ...
    void keyPressedEnter() {
        if (!gameFinished) {
            ...
            if (activeColumn == cells[activeRow].length - 1 &&
                    ...
                    wordleDatabaseConnection.isValidWord(getCurrentGuess())) {
    ...
    private String getCurrentGuess() {
        String guess = "";
        for (int i = 0; i < cells[activeRow].length; i++) {
            guess = guess + cells[activeRow][i].getStoredCharacter();
        }
        return guess;
    }

    protected boolean checkActiveRowAgainstWord() {
        return getCurrentGuess().equals(wordToGuess);
    }
