...
public class Board {
    ...
    public Board(){
        ...
        if(setupStage == 1)
        {
            ...
            try (BufferedReader br = new BufferedReader(new FileReader("resources/data.txt"))) {
                ...
                while ((line = br.readLine()) != null) {
                    if (line.length() == 4 && line.matches("^[a-zA-Z]+$")){ //check if 4 letters
                        wordleDatabaseConnection.addValidWord(i,line.toLowerCase()); // lowercase the word if there are caps
                        i++;

                    }//else do nothing and move to next line with current id held
                }
                numberOfWords = i;
                setupStage = 2;
            }catch(IOException e)
        ...
        String theWord = wordleDatabaseConnection.getWordAtIndex(wordIndexRandomiser());
    ...
    int wordIndexRandomiser(){
        int min = 1;
        int max = numberOfWords;
        return new Random().nextInt((max - min) + 1) + min; //min max inclusive
    }     
    ...
    public void keyPressed(KeyEvent e){
        ...
        if(e.getKeyCode() == KeyEvent.VK_ESCAPE){
            ...
            secretWordIndex = wordIndexRandomiser() % numberOfWords;
